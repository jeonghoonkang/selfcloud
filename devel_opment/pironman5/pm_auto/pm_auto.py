...
class PMAuto():
    ...
    def onshot(self):
        ...
        if self.oled is not None and self.oled.is_ready():
            ...
            print("oled is not ready, fails to run")
...
class OLEDAuto():
    ...
    @log_error
    def handle_oled(self, data): #OLEDAuto handle_oled, start flow
        ...
        #jhkang 
        ip_rect_left =      self.Rect(66,  0, 64, 10) 
        ip_rect_right =     self.Rect(88,  0, 90, 10)  
        co2_disp =         self.Rect(66,  17, 88, 10)

        LEFT_AREA_X = 18
        # cpu usage
        # self.oled.draw_text('CPU', LEFT_AREA_X, 0, align='center')
        self.oled.oled.draw_pieslice_chart(cpu_usage, LEFT_AREA_X, 27, 15, 180, 0)
        self.oled.oled.draw_text(f'{cpu_usage} %', LEFT_AREA_X, 27, align='center')
        # cpu temp
        temp = cpu_temp_c if self.temperature_unit == 'C' else cpu_temp_f
        self.oled.oled.draw_text(f'{temp:.1f}°{self.temperature_unit}', LEFT_AREA_X, 37, align='center')
        self.oled.oled.draw_pieslice_chart(cpu_temp_c, LEFT_AREA_X, 48, 15, 0, 180)
        # RAM
        ...
        # Disk
        self.oled.oled.draw_text(f'DISK: {disk_used}/{disk_total} {disk_unit}', *disk_info_rect.coord())
        self.oled.oled.draw_bar_graph_horizontal(disk_percent, *disk_rect.coord(), *disk_rect.size())
        ...
        if self.disp_date == None :
            self.disp_date = str(now.date())
            self.disp_date = self.disp_date[:10]

        self.oled.oled.draw_text(self.disp_date, 1, 0, align='left')

        self.oled.oled.draw_text(ip + " IP", *ip_rect_left.coord(), align='left')
        self.oled.oled.draw_text("                        ", *ip_rect_left.coord(), align='left')

       #jhkang add showing CO2 value
        co2_val = "777"
        self.oled.oled.draw_text(co2_val, *co2_disp.coord(), align='left')   
    ...
    def run(self): #jhkang OLEDAuto run
        ...
        # exit("##### here is the onshot start point")
        data = self.get_data()

        print ("sonno method : ", dir(self.oled))

        # exit("##### here is check point where to start the code ")
